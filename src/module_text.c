...
static const TextVertex vertices[] = {
 {-0.25f, 0.25f, 0.0f, 1.0f}, // Bottom-left
 {0.25f, 0.25f, 1.0f, 1.0f}, // Bottom-right
 {-0.25f, 0.45f, 0.0f, 0.0f}, // Top-left
 {0.25f, 0.45f, 1.0f, 0.0f} // Top-right
};
...
static VkResult createTextureImage(VulkanContext *vulkanContext, SDL_Surface *surface, VkImage *image, VkDeviceMemory *imageMemory) {
 ...
 if (createBuffer(vulkanContext->device, vulkanContext->physicalDevice, imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &stagingBuffer, &stagingBufferMemory) != VK_SUCCESS) {
 ...
 .extent = { (uint32_t)surface->w, (uint32_t)surface->h, 1 },
 .mipLevels = 1,
 .arrayLayers = 1,
 ...
 copyBufferToImage(commandBuffer, stagingBuffer, *image, surface->w, surface->h);
...
bool text_init(VulkanContext *vulkanContext, TextContext *textContext) {
 ...
 if (createBuffer(vulkanContext->device, vulkanContext->physicalDevice, bufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &textContext->vertexBuffer, &textContext->vertexBufferMemory) != VK_SUCCESS) {
 return false;
 }
 void *data;
 ...
 if (createBuffer(vulkanContext->device, vulkanContext->physicalDevice, bufferSize, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &textContext->indexBuffer, &textContext->indexBufferMemory) != VK_SUCCESS) {
 return false;
 }
